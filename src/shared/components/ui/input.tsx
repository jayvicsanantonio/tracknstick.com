...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        'flex h-9 w-full min-w-0 rounded-md border-2 px-3 py-1 text-base shadow-xs outline-none',
        'border-(--color-border-primary) bg-(--color-surface) text-(--color-foreground) placeholder:text-(--color-text-secondary)',
        'focus:border-(--color-ring) focus-visible:ring-2 focus-visible:ring-(--color-ring)/40',
