      ...
      >
        {daysOfWeek.map((day) => (
          <ToggleGroupItem
            ...
            aria-label={day}
            title={day}
            className="data-[state=off]:bg-(--color-surface-tertiary) data-[state=off]:text-(--color-text-secondary) data-[state=off]:hover:bg-(--color-hover-brand) data-[state=off]:hover:text-(--color-text-inverse) data-[state=on]:bg-(--color-brand-primary) data-[state=on]:text-(--color-text-inverse) dark:data-[state=off]:bg-(--color-brand-light) dark:data-[state=off]:text-(--color-brand-text-light) dark:data-[state=off]:hover:bg-(--color-brand-primary) dark:data-[state=on]:bg-(--color-brand-primary) h-8 w-8 rounded-full text-xs sm:h-10 sm:w-10 sm:text-sm"
          >
            {day[0]}
          </ToggleGroupItem>
        ))}
